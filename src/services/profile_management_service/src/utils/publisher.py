...
@retry(wait=wait_fixed(2), stop=stop_after_attempt(5))
async def publish_profile__approved(message):
    """Publishes a profile approved message to RabbitMQ with retry handling."""
    try:
        connection = await aio_pika.connect_robust(Settings.Config.RABBITMQ_URL)
        async with connection:
            channel = await connection.channel()
            await channel.declare_queue(QueueEventNames.profile_approved, durable=True)
            ...
            await channel.default_exchange.publish(
                ...
                routing_key=QueueEventNames.profile_approved
            )
        logging.info(f"Profile approved Event sent successfully for user_id:  {message.get('user_id')}")
    except Exception as e:
        logging.error(f"Failed to send profile approved event: {e}")
        raise  # Re-raise to trigger retry
