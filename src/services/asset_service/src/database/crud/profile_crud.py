...
from src.database.models.application_model import ApplicationModel
...
from src.database.crud.producer_file_crud import get_all_profile_files_by_email
...
async def create_profile(db, data):
    ...
    result = await db.producer_applications.insert_one(data)
...
async def generate_ai_profile(db, producer_id, profile_data):
    ...
    profile_files = await get_all_profile_files_by_email(db, profile_data['email'])
