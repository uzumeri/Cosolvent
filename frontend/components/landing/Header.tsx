		...
		>
			<div className="container mx-auto flex h-20 items-center justify-between px-4 md:px-6">
				<Link href="/" className="flex items-center gap-2">
					...
					<span
						className={`text-2xl font-bold font-serif transition-colors ${scrolled ? "text-foreground" : "text-white"}`}
					>
						PrairieGrain
					</span>
				</Link>
				<nav className="hidden items-center gap-8 md:flex">
					<Link
						href="#features"
						className={`text-base font-medium transition-colors ${scrolled ? "text-foreground/70 hover:text-foreground" : "text-stone-300 hover:text-white"}`}
					>
						Why PGP?
					</Link>
					<Link
