...
export default function SignUpPage() {
	...
	const inputStyles =
		"bg-white/5 border-white/20 text-white placeholder:text-stone-400 focus-visible:ring-primary/50";
				...
				<AnimatePresence mode="wait">
					...
					>
						{currentStep === 0 && (
							...
							>
								...
								<div className="space-y-2">
									...
									<Input type="password" required className={inputStyles} />
								</div>
								<div className="space-y-2">
						...
						{currentStep === 1 && (
							...
							>
								<div className="space-y-2">
									<Label className="text-stone-300">Select Your Role</Label>
									<div className="grid grid-cols-2 gap-4">
										<RoleCard
											icon={<User />}
											title="I'm a Buyer"
											description="Source quality grain"
											selected={role === "buyer"}
								...
								<div className="flex gap-4">
									<PGPButton
										...
										variant="outline"
										onClick={handlePrev}
										className="w-full text-white border-white/50 hover:bg-white/10"
									>
										Back
									</PGPButton>
									<PGPButton type="submit" className="w-full">
										Continue
									</PGPButton>
								</div>
							</form>
						...
						{currentStep === 2 && (
							<div className="text-center">
								<div className="flex justify-center mb-4">
									<div className="h-16 w-16 bg-green-500/20 text-green-300 rounded-full flex items-center justify-center border border-green-400">
										<Check className="h-8 w-8" />
									</div>
								</div>
								<p className="text-stone-300 mb-6">
									Please check your inbox to complete your registration and gain
									access to the marketplace.
								</p>
								<Link href="/login">
									<PGPButton className="w-full sm:w-1/2 mx-auto" size="lg">
										Go to Login
									</PGPButton>
								</Link>
							</div>
...
const RoleCard = ({
	...
	<button
		type="button"
		onClick={onClick}
		className={`p-4 rounded-lg cursor-pointer transition-all duration-200 text-left w-full ${
			selected
	...
	>
		<div
			className={`mb-2 h-10 w-10 flex items-center justify-center rounded-full transition-colors ${
				selected
